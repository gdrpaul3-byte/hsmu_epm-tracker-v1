...
Point = Tuple[int, int]
ZoneClassifier = Callable[
    [Point, np.ndarray, List[str], List[np.ndarray], List[Point]],
    Optional[str],
]
...
def default_video_path() -> str:
    return os.path.join(project_root(), "tracking", "epm", "m1_epm.mp4")


def default_roi_path() -> str:
    return os.path.join(project_root(), "tracking", "epm", "roi.json")


EPM_ZONE_SEQUENCE: List[Tuple[str, str]] = [
    ("center", "Center (교차 중앙)"),
...
def _zone_fill_color(name: str) -> Tuple[int, int, int]:
    low = name.lower()
    if "center" in low:
        return (0, 165, 255)
    if "open" in low:
...
def _zone_outline_color(name: str) -> Tuple[int, int, int]:
    low = name.lower()
    if "center" in low:
        return (20, 20, 20)
    if "open" in low:
...
def save_zone_plots(
    ...
    try:
        ...
        def time_seconds(entry: Dict[str, object]) -> float:
            val = entry.get("time_ms", 0.0)
            return float(val) if isinstance(val, float) else float(val) / 1000.0

        remaining = set(stats.keys())
        agg: List[Tuple[str, float]] = []

        if "center" in stats:
            agg.append(("center", time_seconds(stats["center"])))
            remaining.discard("center")

        open_names = [n for n in list(remaining) if "open_arm" in n.lower()]
        open_total = sum(time_seconds(stats[n]) for n in open_names)
        for n in open_names:
            remaining.discard(n)
        if open_total > 0:
            agg.append(("open arms", open_total))

        closed_names = [n for n in list(remaining) if "closed_arm" in n.lower()]
        closed_total = sum(time_seconds(stats[n]) for n in closed_names)
        for n in closed_names:
            remaining.discard(n)
        if closed_total > 0:
            agg.append(("closed arms", closed_total))

        for name in sorted(remaining):
            agg.append((name, time_seconds(stats[name])))

        if not agg:
            return

        labels = [name for name, _ in agg]
        totals = [val for _, val in agg]
        total_sum = sum(totals) or 1.0
        percentages = [(val / total_sum) * 100.0 for val in totals]

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
        ax1.bar(labels, totals, color="#4e79a7"); ax1.set_title("Total time (s) / 총 체류시간"); ax1.set_ylabel("s")
        ax2.bar(labels, percentages, color="#d37295"); ax2.set_title("Percentage (%) / 비율"); ax2.set_ylabel("%")
        title_meta = ""
        if meta:
        ...
        img_path = os.path.splitext(export_csv)[0] + "_zone_stats.png" if export_csv else os.path.join(project_root(), "tracking", "epm", "zone_stats.png")
...
def default_calibration_path() -> str:
    return os.path.join(project_root(), "tracking", "epm", "calibration.json")
...
def default_zones_path() -> str:
    return os.path.join(project_root(), "tracking", "epm", "zones.json")
...
def load_reference_gray(video_path: str, frame_idx: int) -> Optional[np.ndarray]:
...
def build_zone_equalizer(zones: Optional[List[Dict]], ref_gray: Optional[np.ndarray]) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    if not zones or ref_gray is None:
        return None
    h, w = ref_gray.shape[:2]
    scale_map = np.ones((h, w), dtype=np.float32)
    offset_map = np.zeros((h, w), dtype=np.float32)
    stats: List[Tuple[np.ndarray, float, float]] = []
    for z in zones:
        ...
        if cv2.countNonZero(mask) == 0:
            continue
        mean = cv2.mean(ref_gray, mask=mask)[0]
        _, stddev = cv2.meanStdDev(ref_gray, mask=mask)
        std = float(stddev[0][0])
        stats.append((mask, float(mean), std))
    if not stats:
        return None
    target_mean = max(s[1] for s in stats)
    positive_std = [s[2] for s in stats if s[2] > 1e-3]
    target_std = max(positive_std) if positive_std else 1.0
    for mask, mean, std in stats:
        gain = target_std / std if std > 1e-3 else 1.0
        offset = target_mean - mean * gain
        mask_bool = mask > 0
        scale_map[mask_bool] = gain
        offset_map[mask_bool] = offset
    return scale_map, offset_map


def _contour_coverage_fraction(contour: np.ndarray, poly: np.ndarray, max_samples: int = 60) -> float:
    if contour is None or contour.size == 0:
        return 0.0
    pts = contour.reshape(-1, 2)
    step = max(1, len(pts) // max(1, max_samples))
    inside = 0
    total = 0
...
def classify_epm_zone(
    ...
    """Assign zone when most of the body (contour) is inside a polygon."""
    has_center_poly = any("center" in name.lower() for name in zone_names)
    best_name: Optional[str] = None
    best_score = 0.0
    for name, poly in zip(zone_names, zone_polys):
        frac = _contour_coverage_fraction(contour, poly)
        if frac > best_score:
            best_score = frac
            best_name = name
    if best_name:
        target = 0.5 if "center" in best_name.lower() else 0.70
        if best_score >= target:
            return best_name
    # fallback to centroid-in-polygon when coverage not met
    for name, poly in zip(zone_names, zone_polys):
        if cv2.pointPolygonTest(poly, (float(centroid[0]), float(centroid[1])), False) >= 0:
            return name
    if roi_pts:
        roi_poly = np.array(roi_pts, np.int32)
        if cv2.pointPolygonTest(roi_poly, (float(centroid[0]), float(centroid[1])), False) >= 0:
            return "center" if not has_center_poly else "maze"
    return None
...
def process_stream(
    ...
    reference_bg_gray: Optional[np.ndarray] = None,
    zone_equalizer: Optional[Tuple[np.ndarray, np.ndarray]] = None,
) -> None:
    mask = None
    path_pts: List[Point] = []
    path_ts: List[float] = []
    ...
    # Use background subtractor to adapt changing illumination (unless static reference diff)
    use_ref_diff = method == "ref" and reference_bg_gray is not None
    if method == "ref" and reference_bg_gray is None:
        print("[WARN] Reference background requested but unavailable; falling back to MOG2.")
    bg = cv2.createBackgroundSubtractorMOG2(history=500, varThreshold=16, detectShadows=False) if not use_ref_diff else None
    ...
    eq_offset: Optional[np.ndarray] = None
    if zone_equalizer is not None:
        eq_scale, eq_offset = zone_equalizer
    if show:
    ...
    while True:
        ...
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        gray = cv2.GaussianBlur(gray, (5, 5), 0)
        if method == "bright" and eq_scale is not None and eq_offset is not None:
            if eq_scale.shape == gray.shape and eq_offset.shape == gray.shape:
                adj = gray.astype(np.float32)
                adj = adj * eq_scale + eq_offset
                gray = np.clip(adj, 0, 255).astype(np.uint8)
        if method == "bright":
        ...
        elif use_ref_diff and reference_bg_gray is not None:
            diff = cv2.absdiff(gray, reference_bg_gray)
            if bright_thresh > 0:
                _, fg = cv2.threshold(diff, bright_thresh, 255, cv2.THRESH_BINARY)
            else:
                _, fg = cv2.threshold(diff, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        else:
            fg = bg.apply(gray) if bg is not None else gray.copy()
        ...
        if zone_polys:
            for name, poly in zip(zone_names, zone_polys):
                fill = _zone_fill_color(name)
                stroke = _zone_outline_color(name)
                cv2.fillPoly(zones_layer_recent, [poly], fill)
                cv2.fillPoly(zones_layer_full, [poly], fill)
                cv2.polylines(zones_layer_recent, [poly], True, (0, 0, 0), 3)
                cv2.polylines(zones_layer_recent, [poly], True, stroke, 1)
                cv2.polylines(zones_layer_full, [poly], True, (0, 0, 0), 3)
                cv2.polylines(zones_layer_full, [poly], True, stroke, 1)
                M = cv2.moments(poly)
                if M["m00"] != 0:
                    zx = int(M["m10"] / M["m00"])
                    zy = int(M["m01"] / M["m00"])
    ...
    # rollback: no lost-counter reset logic
            if heatmap is not None and 0 <= cy < heatmap.shape[0] and 0 <= cx < heatmap.shape[1]:
                heatmap[cy, cx] += 1
            shade_color = (80, 60, 220)
            cv2.drawContours(overlay_recent, [contour], -1, (255, 255, 255), 3)
            cv2.drawContours(overlay_full, [contour], -1, (255, 255, 255), 3)
            cv2.drawContours(overlay_recent, [contour], -1, (255, 0, 0), 1)
            cv2.drawContours(overlay_full, [contour], -1, (255, 0, 0), 1)
            cv2.circle(overlay_recent, (cx, cy), 7, (0, 140, 255), -1)
            cv2.circle(overlay_full, (cx, cy), 7, (0, 140, 255), -1)
            ...
            if zone_polys:
                if zone_classifier is not None:
                    zone_label = zone_classifier((cx, cy), contour, zone_names, zone_polys, roi_pts)
                ...
                if zone_label is None and cv2.pointPolygonTest(np.array(roi_pts, np.int32), (float(cx), float(cy)), False) >= 0:
                    zone_label = "center" if not has_center_poly else "maze"
        ...
        if len(path_pts) > 1:
            ...
            for i in range(1, len(path_pts)):
                cv2.line(overlay_full, path_pts[i - 1], path_pts[i], (0, 80, 0), 5)
            ...
            for i in range(max(1, start_i), len(path_pts)):
                cv2.line(overlay_recent, path_pts[i - 1], path_pts[i], (0, 120, 0), 4)

        # soften body shading
        # blend zones first (under paths), then paths overlays
        vis_recent = cv2.addWeighted(vis_recent, 1.0, zones_layer_recent, 0.18, 0)
        vis_full = cv2.addWeighted(vis_full, 1.0, zones_layer_full, 0.18, 0)
        vis_recent = cv2.addWeighted(vis_recent, 1.0, overlay_recent, 0.8, 0)
        vis_full = cv2.addWeighted(vis_full, 1.0, overlay_full, 0.6, 0)
        active_zone = current_zone or "none"
        zone_label_text = f"Zone: {active_zone.replace('_', ' ')}"
        _draw_text(vis_recent, zone_label_text, (10, 90), font_path, font_size=40, color=(0, 200, 255))
        _draw_text(vis_full, zone_label_text, (10, 90), font_path, font_size=40, color=(0, 200, 255))
        if show:
    ...
    if zones is not None or stats:
        try:
            ...
            if export_csv:
                ...
                out_path = os.path.join(project_root(), "tracking", "epm", "zones_summary.csv")
    ...
    # rollback: no overall distance/velocity summary export
        ...
        try:
            ...
            import matplotlib.pyplot as plt  # type: ignore
            if zone_polys:
                labels = [key for key, _ in EPM_ZONE_SEQUENCE if key in stats]
                if not labels:
    ...
    try:
        if first_bg is not None and len(path_pts) > 1:
            ...
            if zone_polys:
                for name, poly in zip(zone_names, zone_polys):
                    low = name.lower()
                    color = (0, 165, 255) if low == "center" else (255, 200, 0)
                    if "open_arm" not in low:
            ...
            base = os.path.splitext(export_csv)[0] if export_csv else os.path.join(project_root(), "tracking", "epm", "m1_epm")
            title_meta = ""
            if meta:
                title_meta = f"{meta.get('name','')} / {meta.get('student_id','')} / {meta.get('mouse_id','')}"
...
def interactive_zone(frame: np.ndarray, name: str, preset: Optional[List[Point]] = None,
    ...
    while True:
        ...
        try:
            ...
            attempt += 1
            if attempt >= 2:
                print(f"[ERROR] Unable to open zone editor window ({e}).")
                return None
    while True:
        vis = draw_roi(state.image, state.points, state.closed, font_path)
...
def define_epm_zones(frame: np.ndarray, font_path: Optional[str], existing: Optional[List[Dict]] = None) -> List[Dict]:
    """Guide the user through defining center + four arm zones."""
    existing_map = {}
    if existing:
        for z in existing:
            pts = z.get("points")
            if pts and len(pts) >= 3:
                existing_map[str(z.get("name"))] = pts
    print("[INFO] Elevated Plus Maze zone definition:")
    print("       Define center, open arms (2), closed arms (2).")
    print("       (If you skip center, any ROI area outside arms will be treated as center automatically.)")
    print("       Enter redraws, 'k' keeps previous polygon (if exists), 's' skips.")
    zones: List[Dict] = []
    for key, label in EPM_ZONE_SEQUENCE:
        preset = existing_map.get(key)
        prompt = f"{label} ({key}) - redraw? [Enter=y / k keep / s skip]: "
        ans = input(prompt).strip().lower()
        if ans == 'k' and preset:
            zones.append({"name": key, "points": preset})
            continue
        if ans == 's':
            continue
        zpts = interactive_zone(frame, name=label, preset=preset, font_path=font_path)
        if zpts and len(zpts) >= 3:
            zones.append({"name": key, "points": zpts})
    return zones
    ...
    while True:
        ...
        if zones:
            ...
            for name, poly in zip(zone_names, zone_polys):
                fill = _zone_fill_color(name)
                stroke = _zone_outline_color(name)
                cv2.fillPoly(overlay_left, [poly], fill)
                cv2.polylines(overlay_left, [poly], True, (0, 0, 0), 3)
                cv2.polylines(overlay_left, [poly], True, stroke, 1)
                M = cv2.moments(poly)
                if M["m00"] != 0:
                    cx = int(M["m10"] / M["m00"])
                    cy = int(M["m01"] / M["m00"])
        ...
        if zones:
            ...
            for name, poly in zip(zone_names, zone_polys):
                stroke = _zone_outline_color(name)
                cv2.polylines(overlay_right, [poly], True, stroke, 2)
            # draw ROI edge too
...
def main():
    parser = argparse.ArgumentParser(description="Elevated Plus Maze (EPM) mouse tracker with ROI and pre-play start selection")
    parser.add_argument("--video", type=str, help="Path to a video file (or resolved from --mouse-id)")
    parser.add_argument("--mouse-id", type=str, default=None, help="Mouse ID to resolve video (searches tracking/epm/*{ID}*.mp4)")
    ...
    parser.add_argument("--define-zones", action="store_true", help="Open zone editor to redraw center/open/closed arm zones")
    ...
    parser.add_argument("--method", type=str, choices=["bg", "bright", "ref"], default="bg", help="Detection method: adaptive bg (bg), bright threshold, or static reference diff (ref)")
    parser.add_argument("--bright-thresh", type=int, default=0, help="Threshold (0=Otsu) when method=bright")
    parser.add_argument("--ref-frame", type=int, default=None, help="Frame index used to capture reference background when method=ref (default: start frame)")
    ...
    vid_path = args.video
    if not vid_path:
        root = os.path.join(project_root(), "tracking", "epm")
        if mouse_id:
            candidates = []
            try:
    ...
    # Load or define zones (center + 4 arms)
    need_zone_redraw = args.define_zones or roi_redrawn
    zones = load_zones(args.zones)
    if zones is None:
        zones = []
        need_zone_redraw = True
    if need_zone_redraw:
        new_zones = define_epm_zones(first, font_path, existing=zones)
        if new_zones:
            zones = new_zones
            try:
            ...
            print("[WARN] No EPM zones defined; tracking will continue without zone stats.")
    ...
    reference_bg = None
    bright_equalizer = None
    if args.method == "ref":
        ref_idx = args.ref_frame if args.ref_frame is not None else start_frame
        reference_bg = load_reference_gray(vid_path, ref_idx)
        if reference_bg is not None:
            print(f"[INFO] Captured reference background at frame {ref_idx}")
        else:
            print("[WARN] Failed to capture reference background; will fall back to adaptive BG.")
    elif args.method == "bright":
        ref_idx = args.ref_frame if args.ref_frame is not None else start_frame
        ref_gray = load_reference_gray(vid_path, ref_idx)
        if ref_gray is not None:
            eq = build_zone_equalizer(zones, ref_gray)
            if eq is not None:
                bright_equalizer = eq
                print(f"[INFO] Brightness equalizer built from frame {ref_idx}")
            else:
                print("[WARN] Could not build zone equalizer; proceeding without per-zone gains.")
        else:
            print("[WARN] Failed to capture reference frame for bright equalization.")
    ...
    process_stream(
        ...
        zone_classifier=classify_epm_zone,
        reference_bg_gray=reference_bg,
        zone_equalizer=bright_equalizer,
    )
